...
import { createCoupon, findCouponByCode, getCouponById, updateCouponById } from '../core/_requests'
import axios from 'axios'
import { useSelector } from 'react-redux'
import { USERURL } from '../core/_requests'
import ToastComp from '../../../conferenceQuiz/userList/ToastComp'
import Editor from '../../../quizophyWebsite/editor/Editor'

const CreateUpdateCouponUserPanel = () => {
    ...
    const getAllUsers=async ()=>{
        const {data}=await axios.get(`${USERURL}/getAllRegisteredUsers/users`)
        if(data?.success){
            let temp=[...users]
      ...
      const createCouponData=async ()=>{
        try{
            if(couponDetail?.coupon_name?.trim()=="" || !couponDetail?.coupon_name?.trim()){
             ...
             else if(couponDetail?.user_id?.length<=0){
                    ...
                    <div className="row" style={{margin:"auto",display:"flex",justifyContent:"space-between"}}>
                        ...
                        <div className="col-6 p-3" style={{boxShadow: "rgba(0, 0, 0, 0.35) 0px 5px 15px"}}>
                           ...
                           <div className="fv-plugins-message-container mb-1 invalid-feedback"></div>
                                ...
                                <select value={couponDetail?.visible_on_panel==1?"YES":"NO"} className="form_style form-control fw-bolder form-select form-control"  onChange={(e:any)=>{
                                     if(e?.target?.value=="YES"){
                                        setCouponDetail({...couponDetail,visible_on_panel:1})
                                     }else{
                                        setCouponDetail({...couponDetail,visible_on_panel:0})
                                     }
                                }}>
                                    <option disabled={true}>Select</option>
                              ...
                              <div className="fv-plugins-message-container mb-1 invalid-feedback"></div>
                                ...
                                <Select
                                    ...
                                    value={couponDetail?.user_id}
                                    onChange={(e: any, i: any) => {
                                        // console.log(e)
                                        setCouponDetail({...couponDetail,user_id:e})
                                    // setQuizDetail({...quizDetail,language:e})
                                    }}
                              />   
                              <div className="fv-plugins-message-container mb-1 invalid-feedback"></div>
                                ...
                                <div className="col-md-12 fv-row fv-plugins-icon-container mt-3">
                                <label className="required fs-6 fw-semibold text-primary">Reseller Payable Amount</label>
                             
                                <input type="number" className="form_style form-control form-control-solid fw-bolder" style={{background:"#f2f2f2"}}  placeholder="Reseller Payable Amount" name="first-name" value={couponDetail?.reseller_payable_amount} onChange={(e:any)=>{
                                   setCouponDetail({...couponDetail,reseller_payable_amount:e?.target?.value})
                                }}/>
                                 <div className="fv-plugins-message-container mb-1 invalid-feedback"></div>
                                </div>
                                ...
                                <div className="col-md-12 fv-row fv-plugins-icon-container mt-3">
                                <label className="required fs-6 fw-semibold text-primary">Reseller Coupon</label>
                             
                                <input type="checkbox" className="form-check-input ms-2"  placeholder="Reseller Coupon" name="first-name" checked={couponDetail?.reseller==1?true:false} onChange={(e:any)=>{
                                   setCouponDetail({...couponDetail,reseller:e?.target?.checked?1:0})
                                }}/>
                                 <div className="fv-plugins-message-container mb-1 invalid-feedback"></div>
                                </div>
                        </div>
...
export default CreateUpdateCouponUserPanel