...
import { QUIZOPHY_PODIUM_API_URL } from '../../quizophyWebsite/ApiUrl';
...
const CreateUpdateBlog = () => {
    ...
    const [blogThumbnail,setblogThumbnail]=useState<any>("")
...
const createNewBlog=async ()=>{
       if(createBlog?.title?.trim()===""){
        ...
        ToatComp({message:"Please Upload Blog Image",type:"Error"})
       }
       else if(blogThumbnail?.trim()=="" || !blogThumbnail){
        ToatComp({message:"Please Upload Blog Thumbnail",type:"Error"})
       }
       else if(createBlog?.category?.trim()===""){
        ToatComp({message:"Please Select Blog Category",type:"Error"})
       }
      else{
        const {data}=await axios.post(`${QUIZOPHY_PODIUM_API_URL}/blog/add`,{...createBlog,draft:0,blog_thumbnail:blogThumbnail,author:"Quizophy",image:blogImage,slug:blogSlug,description:value})
         if(data?.success){
    ...
    const saveBlogInDraft=async ()=>{
        setCreateBlog({...createBlog,draft:1})
        const {data}=await axios.post(`${QUIZOPHY_PODIUM_API_URL}/blog/add`,{...createBlog,draft:1,blog_thumbnail:blogThumbnail,author:"Quizophy",image:blogImage,slug:blogSlug,description:value})
        if(data?.success){
           ToatComp({message:"Blog Saved in Draft Successfully",type:"Success"})
           navigate("/conference-quiz/blog")
        }      
   }
   const saveBlogInDraftEdit=async ()=>{
     setEditBlog({...editBlog,draft:1})
     const {data}=await axios.put(`${QUIZOPHY_PODIUM_API_URL}/blog/updateBlog/detail/${params?.id}`,{...editBlog,draft:1,blog_thumbnail:blogThumbnail,image:blogImage,author:"Quizophy",slug:blogSlug,description:value})
     if(data?.success){
    ...
    const updateBlogData=async ()=>{
       if(editBlog?.title?.trim()===""){
        ...
        const {data}=await axios.put(`${QUIZOPHY_PODIUM_API_URL}/blog/updateBlog/detail/${params?.id}`,{...editBlog,draft:0,image:blogImage,blog_thumbnail:blogThumbnail, author:"Quizophy",slug:blogSlug,description:value})
         if(data?.success){
            if(createNewCategory){
                saveBlogCategory()
            }
   ...
   const getBlogById=async ()=>{
       const {data}=await axios.get(`${QUIZOPHY_PODIUM_API_URL}/blog/admin/${params.id}`)
       if(data?.success){
        setblogImage(data?.data?.image)
        setblogThumbnail(data?.data?.blog_thumbnail)
        ...
        .post(`${APIURLQUIZ}/admin/upload-testimonial-image`, fd)
          .then((data: AxiosResponse<any>) => {
              if(data?.data?.success){
                setblogThumbnail(data?.data?.image)
              }
          })
          .catch((err) => {
            console.log(err, 'err')
...
<div className="fv-plugins-message-container mb-1 invalid-feedback"></div></div>
                            ...
                            <div className="col-6">
                                 ...
                                 <h2 className="text-center">Upload Blog Thumbnail</h2>

                                 <div className="p-5 mt-5 bg-white mx-auto shadow text-center" style={{width: 350}}>
                                 {blogThumbnail ? <>
                            <img src={blogThumbnail} style={{ height: "120px", width: "100%",objectFit:"contain" }} />
                            ...
                            : (
                              <Dropzone onDrop={(acceptedFiles) => {
                                uploadImageThumbnail(acceptedFiles)
                              }}>
                            ...
                            <div className="col-12 my-4">
                                 ...
                                 {blogImage ? <>
                            <img src={blogImage} style={{ height: "300px", width: "100%",objectFit:"contain" }} />
                            ...
                            <div className="col-12">
                                <div className="text-center mx-auto my-6">
                                   {params?.id ? <button className="btn btn-success text-white" onClick={(e:any)=>{
                                       e?.preventDefault()
                                       updateBlogData()
                                   }}>Publish Blog</button> :<button className="btn btn-primary text-white" onClick={(e:any)=>{
                                        e?.preventDefault()
                                        createNewBlog()
                                   }}>Publish Blog</button>} 
                                   {params?.id && <button className="btn btn-warning text-white ms-4" onClick={(e:any)=>{
                                        e?.preventDefault()
                                        saveBlogInDraftEdit()
