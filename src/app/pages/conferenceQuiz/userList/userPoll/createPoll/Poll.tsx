...
import { createPoll } from '../core/_requests'
const Poll = () => {
            ...
            <div className="row" >
                     ...
                     <div className="col-12 fv-row fv-plugins-icon-container mt-3" style={{position:"relative"}}>
                      ...
                      <div className="fv-plugins-message-container mb-1 invalid-feedback"></div></div>
                                  ...
                                  <label className="required fs-6 fw-semibold mb-1">Select Poll Type</label>
                                   <Select options={votingType} value={selectedType}
                                      ...
                                      onChange={(e:any)=>{
                                        ...
                                        <div>
                                          {
                                            polldata?.options?.length>0 && (polldata?.poll_type=="Multiple Choice" || polldata?.poll_type=="Yes or No") && polldata?.options?.map((item,index)=>{
                                  ...
                                  <div className="d-flex" style={{flexWrap:"wrap"}}>
                                      {
                                        polldata?.options?.length>0 && (polldata?.poll_type=="Image Poll" || polldata?.poll_type=="Audio Poll" || polldata?.poll_type=="Video Poll") && polldata?.options?.map((item,index)=>{
               ...
               <div style={{margin:"30px auto", textAlign:"center"}}>
                 <button className="btn  m-2 btn-dark btn-sm text-center"  onClick={async ()=>{
                       let checkOptions = polldata.options.some((x) => x.option.trim()=="")
                       if(polldata?.poll_title?.trim()=="" || !polldata?.poll_title?.trim()){
                         ...
                         return
                          ...
                          const {data}=await createPoll({...polldata,thumbnail:pollThumbnail,draft:0})
                            if(data?.success){
                 ...
                 <button className="m-2 btn btn-dark btn-sm text-center" onClick={async ()=>{
                       ...
                       if(polldata?.poll_title?.trim()=="" || !polldata?.poll_title?.trim()){
                         ...
                         return
                          ...
                          const {data}=await createPoll({...polldata,thumbnail:pollThumbnail,draft:1})
                            if(data?.success){
