...
const getUsers = (query: string): Promise<any> => {
  return axios.get(`${EXAM_URL}/exam_types/get/exam?${query}`).then((d: AxiosResponse<any>) => d.data)
}
const getUsersById = (query: string,id:any): Promise<any> => {
  return axios.get(`${EXAM_URL}/get/subPackages/${id}?${query}`).then((d: AxiosResponse<any>) => d.data)
}
